...
IOService *FakeIrisXEFramebuffer::probe(IOService *provider, SInt32 *score) {
    ...
    IOLog("\n");
    IOLog("╔══════════════════════════════════════════════════════════════╗\n");
    IOLog("║       FAKEIRISXE V294 - Masked DMA_CTRL experiment      ║\n");
...
*/
    ...
    IOLog("[V294] Acceleration and AGPM-facing claims held back until execution proof exists\n");
...
*/
    ...
    if (directProofMode) {
        IOLog("(FakeIrisXE) [V294] Stage 4: Minimal bare-DMA FIRST -> Linux fallback\n");
    } else {
    ...
    IOLog("(FakeIrisXE) [V294] Loading firmware (Intel PRM compliant)...\n");
    ...
    IOLog("(FakeIrisXE) [V294] Runtime toggles: diag_full=%u diag_quick=%u direct_proof=%u skip_guc=%u force_guc=%u\n",
    ...
    if (skipGuCInit) {
        IOLog("(FakeIrisXE) [V294] ⚠️ Skipping GuC init (-fakeirisxe-noguc set)\n");
        fGuCEnabled = false;
    ...
    if (!fExeclist) {
        ...
        if (!fExeclist->createHwContext()) {
            ...
            if (directProofMode) {
                IOLog("FakeIrisXEFramebuffer: [V294] Direct proof mode active - skipping legacy RCS/BLT ring warmup path\n");
            } else {
...
{
    IOLog("(FakeIrisXE)[V294] ============================================\n");
    IOLog("(FakeIrisXE)[V294] GPU EXECUTION TEST - DIRECT EXECLIST PROOF\n");
    IOLog("(FakeIrisXE)[V294] ============================================\n");
    if (!fExeclist) {
        IOLog("(FakeIrisXE)[V294] No EXECLIST owner available\n");
        return false;
    }
    IOLog("(FakeIrisXE)[V294] Running one-shot scratch writeback proof on plain -fakeirisxe boot...\n");
    bool success = fExeclist->testBatchSubmission();
    IOLog("(FakeIrisXE)[V294] Direct Execlist proof result: %s\n", success ? "PASS" : "FAIL");
    IOLog("(FakeIrisXE)[V294] ============================================\n");
    return success;
}
...
bool FakeIrisXEFramebuffer::initGuCSystem()
{
    ...
    IOLog("(FakeIrisXE) [V294] Loading Linux TGL GuC v70.1.1 (%u bytes)\n", guc_len);

    if (!guc_bin || guc_len == 0) {
        IOLog("(FakeIrisXE) [V294] GuC firmware not available\n");
        return false;
    }
    
    if (!fGuC->loadGuCFirmware(guc_bin, guc_len)) {
        IOLog("(FakeIrisXE) [V294] GuC firmware load failed\n");
        return false;
    }
    IOLog("(FakeIrisXE) [V294] GuC firmware loaded\n");
