...
IOService *FakeIrisXEFramebuffer::probe(IOService *provider, SInt32 *score) {
    ...
    IOLog("\n");
    IOLog("╔══════════════════════════════════════════════════════════════╗\n");
    IOLog("║    FAKEIRISXE V262 - Direct Execlist Proof Mode      ║\n");
...
*/
    ...
    IOLog("[V262] Acceleration and AGPM-facing claims held back until execution proof exists\n");
...
*/
    ...
    if (directProofMode) {
        IOLog("(FakeIrisXE) [V262] Stage 4 direct-proof mode: skipping legacy gpuPowerOn/createRcsRing preflight\n");
    } else {
    ...
    IOLog("(FakeIrisXE) [V262] Loading firmware (Intel PRM compliant)...\n");
    ...
    IOLog("(FakeIrisXE) [V262] Runtime toggles: diag_full=%u diag_quick=%u direct_proof=%u skip_guc=%u force_guc=%u\n",
    ...
    if (skipGuCInit) {
        IOLog("(FakeIrisXE) [V262] ⚠️ Skipping GuC init (%s)\n",
    ...
    if (!fExeclist) {
        ...
        if (!fExeclist->createHwContext()) {
            ...
            if (directProofMode) {
                IOLog("FakeIrisXEFramebuffer: [V262] Direct proof mode active - skipping legacy RCS/BLT ring warmup path\n");
            } else {
    ...
    IOLog("(FakeIrisXE) start timing: total=%llu us softFails=%u\n",
          ...
          softFailCount);
    IOLog("🏁 FakeIrisXEFramebuffer::start() - Completed (V262, execution still diagnostic)\n");
    return true;
...
bool FakeIrisXEFramebuffer::testGPUExecution()
{
    ...
    IOLog("(FakeIrisXE)[V262] GPU EXECUTION TEST - DIRECT EXECLIST PROOF\n");
    IOLog("(FakeIrisXE)[V262] ============================================\n");

    if (!fExeclist) {
        IOLog("(FakeIrisXE)[V262] ❌ No EXECLIST owner available\n");
        return false;
    }

    IOLog("(FakeIrisXE)[V262] Running one-shot scratch writeback proof on plain -fakeirisxe boot...\n");
    bool success = fExeclist->testBatchSubmission();
    IOLog("(FakeIrisXE)[V262] Direct Execlist proof result: %s\n", success ? "PASS" : "FAIL");
    IOLog("(FakeIrisXE)[V262] ============================================\n");
    return success;
}
