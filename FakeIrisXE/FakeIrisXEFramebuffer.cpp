...
IOService *FakeIrisXEFramebuffer::probe(IOService *provider, SInt32 *score) {
    ...
    IOLog("\n");
    IOLog("╔══════════════════════════════════════════════════════════════╗\n");
    IOLog("║    FAKEIRISXE V284 - Minimal bare-DMA boot (pre-arm!)      ║\n");
...
*/
    ...
    IOLog("[V284] Acceleration and AGPM-facing claims held back until execution proof exists\n");
...
*/
    ...
    // V284: Minimal bare-DMA FIRST → Apple path → Linux path
    ...
    if (directProofMode) {
        IOLog("(FakeIrisXE) [V284] Stage 4: Minimal bare-DMA FIRST → Apple path → Linux fallback\n");
    } else {
    ...
    IOLog("(FakeIrisXE) [V284] Loading firmware (Intel PRM compliant)...\n");
    ...
    IOLog("(FakeIrisXE) [V284] Runtime toggles: diag_full=%u diag_quick=%u direct_proof=%u skip_guc=%u force_guc=%u\n",
    ...
    if (skipGuCInit) {
        IOLog("(FakeIrisXE) [V284] ⚠️ Skipping GuC init (-fakeirisxe-noguc set)\n");
        fGuCEnabled = false;
    ...
    if (!fExeclist) {
        ...
        if (!fExeclist->createHwContext()) {
            ...
            if (directProofMode) {
                IOLog("FakeIrisXEFramebuffer: [V284] Direct proof mode active - skipping legacy RCS/BLT ring warmup path\n");
            } else {
    ...
    IOLog("(FakeIrisXE) start timing: total=%llu us softFails=%u\n",
          ...
          softFailCount);
    IOLog("FakeIrisXEFramebuffer::start() - Completed (V284, Minimal bare-DMA boot)\n");
    return true;
...
{
    IOLog("(FakeIrisXE)[V284] ============================================\n");
    IOLog("(FakeIrisXE)[V284] GPU EXECUTION TEST - DIRECT EXECLIST PROOF\n");
    IOLog("(FakeIrisXE)[V284] ============================================\n");
    if (!fExeclist) {
        IOLog("(FakeIrisXE)[V284] No EXECLIST owner available\n");
        return false;
    }
    IOLog("(FakeIrisXE)[V284] Running one-shot scratch writeback proof on plain -fakeirisxe boot...\n");
    bool success = fExeclist->testBatchSubmission();
    IOLog("(FakeIrisXE)[V284] Direct Execlist proof result: %s\n", success ? "PASS" : "FAIL");
    IOLog("(FakeIrisXE)[V284] ============================================\n");
    return success;
}
