...
static const char* kExeclistVersion = "V267";
...
static void logProofDwords(const char* label, const uint32_t* words, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        IOLog("(FakeIrisXE) [V267] %s[%u] = 0x%08X\n", label, i, words[i]);
    }
}
                                 ...
                                 const char* label)
{
    ...
    if (!gem) {
        IOLog("(FakeIrisXE) [V267] ❌ %s allocation failed\n", label ? label : "CPU page");
        return false;
    }
    ...
    if (!physAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ %s physical address acquisition failed\n", label ? label : "CPU page");
    ...
    if (!md || !md->getBytesNoCopy()) {
        IOLog("(FakeIrisXE) [V267] ❌ %s CPU mapping failed\n", label ? label : "CPU page");
        cleanupProofCpuGem(gem, physAddr);
        return false;
    }
...
static bool buildProofPageTables(RcsProofResources& res)
{
    ...
    if (!scratchPhys) {
        IOLog("(FakeIrisXE) [V267] ❌ Scratch physical address acquisition failed for PPGTT setup\n");
        return false;
    }
    ...
    if (!pml4 || !pdpt || !pd || !pt) {
        IOLog("(FakeIrisXE) [V267] ❌ Page-table CPU pointer missing\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V267]   PPGTT scratch VA: 0x%016llX -> phys 0x%016llX\n",
...
static bool allocateProofResources(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V267] Allocating direct Execlist proof resources...\n");

    res.ringGem = FakeIrisXEGEM::withSize(kProofRingSize, 0);
    if (!res.ringGem) {
        IOLog("(FakeIrisXE) [V267] ❌ Ring allocation failed\n");
        return false;
    }
    res.ringGem->pin();
    res.ringGpuAddr = self->fOwner->ggttMap(res.ringGem) & ~0xFFFULL;
    if (!res.ringGpuAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ Ring GGTT mapping failed\n");
    ...
    if (!res.lrcGem) {
        IOLog("(FakeIrisXE) [V267] ❌ Context allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.lrcGem->pin();
    ...
    if (!res.lrcGpuAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ Context GGTT mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.scratchGem) {
        IOLog("(FakeIrisXE) [V267] ❌ Scratch allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.scratchGem->pin();
    ...
    if (!res.scratchGpuAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ Scratch GGTT mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!scratchCpu) {
        IOLog("(FakeIrisXE) [V267] ❌ Scratch CPU mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    *(volatile uint32_t*)scratchCpu = kProofScratchInitial;
    ...
    if (!res.pml4Gem) {
        IOLog("(FakeIrisXE) [V267] ❌ PML4 allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.pml4Gem->pin();
    ...
    if (!pml4Md || !pml4Md->getBytesNoCopy()) {
        IOLog("(FakeIrisXE) [V267] ❌ PML4 CPU mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.pml4PhysAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ PML4 physical address acquisition failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.csbGpuAddr) {
        IOLog("(FakeIrisXE) [V267] ❌ Shared CSB backing is missing; direct proof must use the staged Execlist owner HWS/CSB page\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V267]   Ring GPU VA:    0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V267]   CTX GPU VA:     0x%016llX\n", (unsigned long long)res.lrcGpuAddr);
    IOLog("(FakeIrisXE) [V267]   Scratch GPU VA: 0x%016llX\n", (unsigned long long)res.scratchGpuAddr);
    IOLog("(FakeIrisXE) [V267]   Shared HWS VA:  0x%016llX\n", (unsigned long long)sharedHwsGpuAddr);
    IOLog("(FakeIrisXE) [V267]   Shared CSB VA:  0x%016llX\n", (unsigned long long)res.csbGpuAddr);
    IOLog("(FakeIrisXE) [V267]   PML4 phys:      0x%016llX\n", (unsigned long long)res.pml4PhysAddr);
    IOLog("(FakeIrisXE) [V267]   PDPT phys:      0x%016llX\n", (unsigned long long)res.pdptPhysAddr);
    IOLog("(FakeIrisXE) [V267]   PD phys:        0x%016llX\n", (unsigned long long)res.pdPhysAddr);
    IOLog("(FakeIrisXE) [V267]   PT phys:        0x%016llX\n", (unsigned long long)res.ptPhysAddr);
    IOLog("(FakeIrisXE) [V267]   Scratch init:   0x%08X\n", kProofScratchInitial);
    return true;
}
...
static bool buildProofCommandStream(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    if (!ringCpu) {
        IOLog("(FakeIrisXE) [V267] ❌ Ring CPU mapping failed\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V267] ========== RCS TEST COMMAND STREAM ==========" "\n");
    IOLog("(FakeIrisXE) [V267]   Packet: MI_STORE_DWORD_IMM_GEN4 | MI_USE_GGTT\n");
    IOLog("(FakeIrisXE) [V267]   Ring GPU VA: 0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V267]   Ring size:   %u bytes\n", kProofRingSize);
    IOLog("(FakeIrisXE) [V267]   Ring head:   0 bytes\n");
    IOLog("(FakeIrisXE) [V267]   Ring tail:   %u bytes\n", res.ringTailBytes);
    IOLog("(FakeIrisXE) [V267]   Ring ctl:    0x%08X\n", res.ringCtl);
...
static void preprimeLegacyRingState(FakeIrisXEExeclist* self, const RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V267] Pre-primed live ring regs:   START=0x%08X HEAD=0x%08X TAIL=0x%08X CTL=0x%08X\n",
          liveStart, liveHead, liveTail, liveCtl);
    IOLog("(FakeIrisXE) [V267] Pre-primed alt RB regs:      START=0x%08X%08X HEAD=0x%08X TAIL=0x%08X\n",
...
static bool buildProofLrc(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    if (!lrcCpu) {
    IOLog("(FakeIrisXE) [V267] ❌ Context CPU mapping failed\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V267] ========== GEN12 RCS CONTEXT OBJECT ==========" "\n");
    IOLog("(FakeIrisXE) [V267]   Context pages: %u total (%u-page render ctx + %u WA pages)\n",
          kProofContextPages, kProofRenderContextPages, kProofWaContextPages);
    IOLog("(FakeIrisXE) [V267]   RegState @ +0x%X LRI0=0x%08X LRI1=0x%08X LRI2=0x%08X LRI3=0x%08X LRI4=0x%08X\n",
          kProofRegStateOffset, regState[1], regState[33], regState[52], regState[65], regState[81]);
    IOLog("(FakeIrisXE) [V267]   CONTEXT_CTL slot: 0x%08X (Linux-style masked restore-inhibit + inhibit-sync-switch)\n",
          regState[kCtxContextControlIndex]);
    IOLog("(FakeIrisXE) [V267]   PDP0 PML4 phys: 0x%016llX\n", (unsigned long long)res.pml4PhysAddr);
    IOLog("(FakeIrisXE) [V267]   IndirectCtx GGTT: 0x%016llX size=%uB offset=0x%03X firstDW=0x%08X\n",
          (unsigned long long)indirectCtxGpuAddr, indirectCtxBytes, regState[kCtxIndirectCtxOffsetIndex], indirectCtx[0]);
    IOLog("(FakeIrisXE) [V267]   PerCtxBB GGTT:   0x%016llX flags=0x%08X firstDW=0x%08X\n",
          (unsigned long long)perCtxBbGpuAddr, regState[kCtxPerCtxBbPtrIndex] & 0xFFFu, perCtxBb[0]);
    IOLog("(FakeIrisXE) [V267]   RING_BASE:       0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V267]   RING_HEAD:       %u\n", regState[kCtxRingHeadIndex]);
    IOLog("(FakeIrisXE) [V267]   RING_TAIL:       %u bytes (qword aligned)\n", regState[kCtxRingTailIndex]);
    IOLog("(FakeIrisXE) [V267]   RING_CTL:        0x%08X\n", regState[kCtxRingCtlIndex]);
    IOLog("(FakeIrisXE) [V267]   Live RPCS:       0x%08X Live CMD_BUF_CCTL: 0x%08X\n", liveRpcs, liveCmdBufCctl);
    IOLog("(FakeIrisXE) [V267]   Live seed:       BBADDR_LO=0x%08X BBADDR_HI=0x%08X MI_MODE=0x%08X\n",
          self->mmioRead32(0x2140u), self->mmioRead32(0x2168u), self->mmioRead32(0x209Cu));
    IOLog("(FakeIrisXE) [V267]   Indirect WA:     AUX_INV + instruction-state-cache invalidate appended\n");
    return true;
}
...
static void buildProofDescriptor(RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V267] ========== CONTEXT DESCRIPTOR ==========" "\n");
    IOLog("(FakeIrisXE) [V267]   DWord0: 0x%08X\n", res.descLo);
    IOLog("(FakeIrisXE) [V267]   DWord1: 0x%08X\n", res.descHi);
    IOLog("(FakeIrisXE) [V267]   Address field: 0x%08X -> GPU VA 0x%016llX\n",
          res.descLo & 0xFFFFF000u,
          (unsigned long long)(res.descLo & 0xFFFFF000u));
    IOLog("(FakeIrisXE) [V267]   Flags: VALID=%u PRIV=%u FORCE_RESTORE=%u ADDR_MODE=%u\n",
    ...
    IOLog("(FakeIrisXE) [V267]   Context pages: %u (descriptor does not encode page count on Gen11+)\n",
          kProofContextPages);
    IOLog("(FakeIrisXE) [V267]   SW context ID: %u EngineClass: %u EngineInstance: %u\n",
...
static bool singleResetAttemptIfNeeded(FakeIrisXEExeclist* self)
{
    ...
    IOLog("(FakeIrisXE) [V267] Pre-submit RCS status=0x%08X GT_ERROR=0x%08X\n", statusBefore, gtErrorBefore);
    if (!haltedBefore && !wedgedBefore) {
        return true;
    }

    IOLog("(FakeIrisXE) [V267] RCS looks halted/wedged; performing one focused reset attempt\n");
    ...
    IOLog("(FakeIrisXE) [V267] Post-reset RCS status=0x%08X GT_ERROR=0x%08X\n", statusAfter, gtErrorAfter);
...
static bool submitProofDescriptor(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V267] Pre-submit ELSP: LO=0x%08X HI=0x%08X STATUS=0x%08X/0x%08X\n",
    ...
    IOLog("(FakeIrisXE) [V267] Post-submit ELSP: LO=0x%08X HI=0x%08X STATUS=0x%08X/0x%08X latched=%u\n",
...
static bool pollProofProgress(FakeIrisXEExeclist* self, RcsProofResources& res, ProofFailureType& failure)
{
    ...
    IOLog("(FakeIrisXE) [V267] ========== EXECUTION POLL ==========" "\n");
    ...
    for (uint32_t poll = 0; poll < 100; ++poll) {
        ...
        if ((poll % 5u) == 0u || scratchValue == res.expectedValue || halted || wedged) {
            IOLog("(FakeIrisXE) [V267] Poll%03u ELSP=%08X/%08X EXE=%08X/%08X RCS H/T/S=%08X/%08X/%08X\n",
                  poll, elspLo, elspHi, execlistStatusLo, execlistStatusHi, rcsHead, rcsTail, rcsStatus);
            IOLog("(FakeIrisXE) [V267]         CSB ctrl=%08X addr=%08X%08X rp=%08X wp_alias=%08X CCID=%08X CTXCTL=%08X\n",
                  csbCtrl, csbAddrHi, csbAddrLo, csbRead, csbWriteAlias, ccid, ctxCtrl);
            IOLog("(FakeIrisXE) [V267]         ACTHD=%08X%08X BBADDR=%08X%08X GT_ERR=%08X SCRATCH=%08X\n",
                  acthdHi, acthdLo, bbAddrHi, bbAddrLo, gtError, scratchValue);
        }

        if (scratchValue == res.expectedValue) {
            IOLog("(FakeIrisXE) [V267] ✅ SUCCESS: scratch changed from 0x%08X to 0x%08X\n",
...
static bool runRcsScratchWriteProof(FakeIrisXEExeclist* self, const char* label)
{
    ...
    IOLog("(FakeIrisXE) [V267] ============================================\n");
    IOLog("(FakeIrisXE) [V267] DIRECT EXECLIST SCRATCH-WRITE PROOF (%s)\n", label ? label : "unknown");
    IOLog("(FakeIrisXE) [V267] ============================================\n");
    ...
    if (!singleResetAttemptIfNeeded(self)) {
        IOLog("(FakeIrisXE) [V267] RCS remained halted after the focused recovery attempt; continuing with submission so the failure can be classified after ELSP/CSB polling\n");
    }

    if (!self->fOwner->forcewakeRenderHold(5000)) {
        IOLog("(FakeIrisXE) [V267] ❌ Failed to acquire forcewake for proof preparation/submission\n");
        failure = EngineHardHalted;
        goto done;
    }
...
done_release:
    ...
    if (!success) {
        IOLog("(FakeIrisXE) [V267] ❌ FAILURE TYPE: %s\n", proofFailureLabel(failure));
    }
