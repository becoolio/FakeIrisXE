...
static const char* kExeclistVersion = "V265";
...
static const uint32_t kProofGen12AuxInvMmio = 0x42C8u;
...
static uint32_t buildProofIndirectCtxCommands(const RcsProofResources& res, uint32_t* indirectCtx)
{
    ...
    auto emitLri = [&](uint32_t reg, uint32_t value, uint32_t& idx) {
        indirectCtx[idx++] = kMiLoadRegisterImmBase | (1u << 16) | 1u;
        indirectCtx[idx++] = reg;
    ...
    emitLri(kProofGen12AuxInvMmio, maskedBitEnable(kProofAuxInvBit), idx);
...
static void logProofDwords(const char* label, const uint32_t* words, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        IOLog("(FakeIrisXE) [V265] %s[%u] = 0x%08X\n", label, i, words[i]);
    }
}
                                 ...
                                 const char* label)
{
    ...
    if (!gem) {
        IOLog("(FakeIrisXE) [V265] ❌ %s allocation failed\n", label ? label : "CPU page");
        return false;
    }
    ...
    if (!physAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ %s physical address acquisition failed\n", label ? label : "CPU page");
    ...
    if (!md || !md->getBytesNoCopy()) {
        IOLog("(FakeIrisXE) [V265] ❌ %s CPU mapping failed\n", label ? label : "CPU page");
        cleanupProofCpuGem(gem, physAddr);
        return false;
    }
...
static bool buildProofPageTables(RcsProofResources& res)
{
    ...
    if (!scratchPhys) {
        IOLog("(FakeIrisXE) [V265] ❌ Scratch physical address acquisition failed for PPGTT setup\n");
        return false;
    }
    ...
    if (!pml4 || !pdpt || !pd || !pt) {
        IOLog("(FakeIrisXE) [V265] ❌ Page-table CPU pointer missing\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V265]   PPGTT scratch VA: 0x%016llX -> phys 0x%016llX\n",
...
static bool allocateProofResources(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V265] Allocating direct Execlist proof resources...\n");

    res.ringGem = FakeIrisXEGEM::withSize(kProofRingSize, 0);
    if (!res.ringGem) {
        IOLog("(FakeIrisXE) [V265] ❌ Ring allocation failed\n");
        return false;
    }
    res.ringGem->pin();
    res.ringGpuAddr = self->fOwner->ggttMap(res.ringGem) & ~0xFFFULL;
    if (!res.ringGpuAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ Ring GGTT mapping failed\n");
    ...
    if (!res.lrcGem) {
        IOLog("(FakeIrisXE) [V265] ❌ Context allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.lrcGem->pin();
    ...
    if (!res.lrcGpuAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ Context GGTT mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.scratchGem) {
        IOLog("(FakeIrisXE) [V265] ❌ Scratch allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.scratchGem->pin();
    ...
    if (!res.scratchGpuAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ Scratch GGTT mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!scratchCpu) {
        IOLog("(FakeIrisXE) [V265] ❌ Scratch CPU mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    *(volatile uint32_t*)scratchCpu = kProofScratchInitial;
    ...
    if (!res.pml4Gem) {
        IOLog("(FakeIrisXE) [V265] ❌ PML4 allocation failed\n");
        releaseProofResources(self, res);
        return false;
    }
    res.pml4Gem->pin();
    ...
    if (!pml4Md || !pml4Md->getBytesNoCopy()) {
        IOLog("(FakeIrisXE) [V265] ❌ PML4 CPU mapping failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.pml4PhysAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ PML4 physical address acquisition failed\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    if (!res.csbGpuAddr) {
        IOLog("(FakeIrisXE) [V265] ❌ Shared CSB backing is missing; direct proof must use the staged Execlist owner HWS/CSB page\n");
        releaseProofResources(self, res);
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V265]   Ring GPU VA:    0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V265]   CTX GPU VA:     0x%016llX\n", (unsigned long long)res.lrcGpuAddr);
    IOLog("(FakeIrisXE) [V265]   Scratch GPU VA: 0x%016llX\n", (unsigned long long)res.scratchGpuAddr);
    IOLog("(FakeIrisXE) [V265]   Shared HWS VA:  0x%016llX\n", (unsigned long long)sharedHwsGpuAddr);
    IOLog("(FakeIrisXE) [V265]   Shared CSB VA:  0x%016llX\n", (unsigned long long)res.csbGpuAddr);
    IOLog("(FakeIrisXE) [V265]   PML4 phys:      0x%016llX\n", (unsigned long long)res.pml4PhysAddr);
    IOLog("(FakeIrisXE) [V265]   PDPT phys:      0x%016llX\n", (unsigned long long)res.pdptPhysAddr);
    IOLog("(FakeIrisXE) [V265]   PD phys:        0x%016llX\n", (unsigned long long)res.pdPhysAddr);
    IOLog("(FakeIrisXE) [V265]   PT phys:        0x%016llX\n", (unsigned long long)res.ptPhysAddr);
    IOLog("(FakeIrisXE) [V265]   Scratch init:   0x%08X\n", kProofScratchInitial);
    return true;
}
...
static bool buildProofCommandStream(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    if (!ringCpu) {
        IOLog("(FakeIrisXE) [V265] ❌ Ring CPU mapping failed\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V265] ========== RCS TEST COMMAND STREAM ==========" "\n");
    IOLog("(FakeIrisXE) [V265]   Packet: MI_STORE_DWORD_IMM_GEN4 | MI_USE_GGTT\n");
    IOLog("(FakeIrisXE) [V265]   Ring GPU VA: 0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V265]   Ring size:   %u bytes\n", kProofRingSize);
    IOLog("(FakeIrisXE) [V265]   Ring head:   0 bytes\n");
    IOLog("(FakeIrisXE) [V265]   Ring tail:   %u bytes\n", res.ringTailBytes);
    IOLog("(FakeIrisXE) [V265]   Ring ctl:    0x%08X\n", res.ringCtl);
...
static void preprimeLegacyRingState(FakeIrisXEExeclist* self, const RcsProofResources& res)
{
    ...
    self->mmioWrite32(kExecRbStartLoReg, (uint32_t)(res.ringGpuAddr & 0xFFFFFFFFULL));
    self->mmioWrite32(kExecRbStartHiReg, (uint32_t)(res.ringGpuAddr >> 32));
    self->mmioWrite32(kExecRbHeadReg, 0u);
    self->mmioWrite32(kExecRbTailReg, res.ringTailBytes);
    ...
    IOLog("(FakeIrisXE) [V265] Pre-primed legacy RB regs: START=0x%08X%08X HEAD=0x%08X TAIL=0x%08X\n",
...
static bool buildProofLrc(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    if (!lrcCpu) {
    IOLog("(FakeIrisXE) [V265] ❌ Context CPU mapping failed\n");
        return false;
    }
    ...
    IOLog("(FakeIrisXE) [V265] ========== GEN12 RCS CONTEXT OBJECT ==========" "\n");
    IOLog("(FakeIrisXE) [V265]   Context pages: %u total (%u-page render ctx + %u WA pages)\n",
          kProofContextPages, kProofRenderContextPages, kProofWaContextPages);
    IOLog("(FakeIrisXE) [V265]   RegState @ +0x%X LRI0=0x%08X LRI1=0x%08X LRI2=0x%08X LRI3=0x%08X LRI4=0x%08X\n",
          kProofRegStateOffset, regState[1], regState[33], regState[52], regState[65], regState[81]);
    IOLog("(FakeIrisXE) [V265]   CONTEXT_CTL slot: 0x%08X (Linux-style masked restore-inhibit + inhibit-sync-switch)\n",
          regState[kCtxContextControlIndex]);
    IOLog("(FakeIrisXE) [V265]   PDP0 PML4 phys: 0x%016llX\n", (unsigned long long)res.pml4PhysAddr);
    IOLog("(FakeIrisXE) [V265]   IndirectCtx GGTT: 0x%016llX size=%uB offset=0x%03X firstDW=0x%08X\n",
          (unsigned long long)indirectCtxGpuAddr, indirectCtxBytes, regState[kCtxIndirectCtxOffsetIndex], indirectCtx[0]);
    IOLog("(FakeIrisXE) [V265]   PerCtxBB GGTT:   0x%016llX flags=0x%08X firstDW=0x%08X\n",
          (unsigned long long)perCtxBbGpuAddr, regState[kCtxPerCtxBbPtrIndex] & 0xFFFu, perCtxBb[0]);
    IOLog("(FakeIrisXE) [V265]   RING_BASE:       0x%016llX\n", (unsigned long long)res.ringGpuAddr);
    IOLog("(FakeIrisXE) [V265]   RING_HEAD:       %u\n", regState[kCtxRingHeadIndex]);
    IOLog("(FakeIrisXE) [V265]   RING_TAIL:       %u bytes (qword aligned)\n", regState[kCtxRingTailIndex]);
    IOLog("(FakeIrisXE) [V265]   RING_CTL:        0x%08X\n", regState[kCtxRingCtlIndex]);
    IOLog("(FakeIrisXE) [V265]   Live RPCS:       0x%08X Live CMD_BUF_CCTL: 0x%08X\n", liveRpcs, liveCmdBufCctl);
    IOLog("(FakeIrisXE) [V265]   Live seed:       BBADDR_LO=0x%08X BBADDR_HI=0x%08X MI_MODE=0x%08X\n",
          self->mmioRead32(0x2140u), self->mmioRead32(0x2168u), self->mmioRead32(0x209Cu));
    IOLog("(FakeIrisXE) [V265]   Indirect WA:     AUX_INV + instruction-state-cache invalidate appended\n");
    return true;
}
...
static void buildProofDescriptor(RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V265] ========== CONTEXT DESCRIPTOR ==========" "\n");
    IOLog("(FakeIrisXE) [V265]   DWord0: 0x%08X\n", res.descLo);
    IOLog("(FakeIrisXE) [V265]   DWord1: 0x%08X\n", res.descHi);
    IOLog("(FakeIrisXE) [V265]   Address field: 0x%08X -> GPU VA 0x%016llX\n",
          res.descLo & 0xFFFFF000u,
          (unsigned long long)(res.descLo & 0xFFFFF000u));
    IOLog("(FakeIrisXE) [V265]   Flags: VALID=%u PRIV=%u FORCE_RESTORE=%u ADDR_MODE=%u\n",
    ...
    IOLog("(FakeIrisXE) [V265]   Context pages: %u (descriptor does not encode page count on Gen11+)\n",
          kProofContextPages);
    IOLog("(FakeIrisXE) [V265]   SW context ID: %u EngineClass: %u EngineInstance: %u\n",
...
static bool singleResetAttemptIfNeeded(FakeIrisXEExeclist* self)
{
    ...
    IOLog("(FakeIrisXE) [V265] Pre-submit RCS status=0x%08X GT_ERROR=0x%08X\n", statusBefore, gtErrorBefore);
    if (!haltedBefore && !wedgedBefore) {
        return true;
    }

    IOLog("(FakeIrisXE) [V265] RCS looks halted/wedged; performing one focused reset attempt\n");
    ...
    IOLog("(FakeIrisXE) [V265] Post-reset RCS status=0x%08X GT_ERROR=0x%08X\n", statusAfter, gtErrorAfter);
...
static bool submitProofDescriptor(FakeIrisXEExeclist* self, RcsProofResources& res)
{
    ...
    IOLog("(FakeIrisXE) [V265] Pre-submit ELSP: LO=0x%08X HI=0x%08X STATUS=0x%08X/0x%08X\n",
    ...
    IOLog("(FakeIrisXE) [V265] Post-submit ELSP: LO=0x%08X HI=0x%08X STATUS=0x%08X/0x%08X latched=%u\n",
...
static bool pollProofProgress(FakeIrisXEExeclist* self, RcsProofResources& res, ProofFailureType& failure)
{
    ...
    IOLog("(FakeIrisXE) [V265] ========== EXECUTION POLL ==========" "\n");
    ...
    for (uint32_t poll = 0; poll < 100; ++poll) {
        ...
        if ((poll % 5u) == 0u || scratchValue == res.expectedValue || halted || wedged) {
            IOLog("(FakeIrisXE) [V265] Poll%03u ELSP=%08X/%08X EXE=%08X/%08X RCS H/T/S=%08X/%08X/%08X\n",
                  poll, elspLo, elspHi, execlistStatusLo, execlistStatusHi, rcsHead, rcsTail, rcsStatus);
            IOLog("(FakeIrisXE) [V265]         CSB ctrl=%08X addr=%08X%08X rp=%08X wp_alias=%08X CCID=%08X CTXCTL=%08X\n",
                  csbCtrl, csbAddrHi, csbAddrLo, csbRead, csbWriteAlias, ccid, ctxCtrl);
            IOLog("(FakeIrisXE) [V265]         ACTHD=%08X%08X BBADDR=%08X%08X GT_ERR=%08X SCRATCH=%08X\n",
                  acthdHi, acthdLo, bbAddrHi, bbAddrLo, gtError, scratchValue);
        }

        if (scratchValue == res.expectedValue) {
            IOLog("(FakeIrisXE) [V265] ✅ SUCCESS: scratch changed from 0x%08X to 0x%08X\n",
...
static bool runRcsScratchWriteProof(FakeIrisXEExeclist* self, const char* label)
{
    ...
    IOLog("(FakeIrisXE) [V265] ============================================\n");
    IOLog("(FakeIrisXE) [V265] DIRECT EXECLIST SCRATCH-WRITE PROOF (%s)\n", label ? label : "unknown");
    IOLog("(FakeIrisXE) [V265] ============================================\n");
    ...
    if (!singleResetAttemptIfNeeded(self)) {
        IOLog("(FakeIrisXE) [V265] RCS remained halted after the focused recovery attempt; continuing with submission so the failure can be classified after ELSP/CSB polling\n");
    }

    if (!self->fOwner->forcewakeRenderHold(5000)) {
        IOLog("(FakeIrisXE) [V265] ❌ Failed to acquire forcewake for proof preparation/submission\n");
        failure = EngineHardHalted;
        goto done;
    }
...
done_release:
    ...
    if (!success) {
        IOLog("(FakeIrisXE) [V265] ❌ FAILURE TYPE: %s\n", proofFailureLabel(failure));
    }
